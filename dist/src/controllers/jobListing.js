...
const JobListing_1 = __importDefault(require("../models/JobListing"));
// CREATE - Create a new job listing
const createJobListing = async (req, res) => {
    try {
        const newJobListing = new JobListing_1.default(req.body);
...
const getAllJobListings = async (req, res) => {
    try {
        const jobListings = await JobListing_1.default.find();
...
const getJobListingById = async (req, res) => {
    ...
    try {
        const jobListing = await JobListing_1.default.findOne({ _id: req.params.id });
        if (!jobListing) {
...
const getJobListingByRef = async (req, res) => {
    try {
        const jobListingByRef = await JobListing_1.default.findOne({ ref: req.params.ref });
        if (!jobListingByRef) {
            return res.status(404).json("Job listing not found!");
        }
...
const updateJobListing = async (req, res) => {
    try {
        const updatedJobListing = await JobListing_1.default.findByIdAndUpdate(req.params.id, req.body, { new: true });
...
const deleteJobListing = async (req, res) => {
    try {
        const deletedJobListing = await JobListing_1.default.findByIdAndDelete(req.params.id);
